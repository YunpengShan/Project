...
<head>
    ...
    <title>Inventory Database Example</title>
    <!-- add your CSS -->
...
<body>
    ...
    <header>
        <h1>Yunpeng Shan Video Games</h1>
        <h2>Search for Games</h2>
    </header>

    <main>
        <p><a href="index.php">Back to Add Records Page</a></p>
        <form method="post">
            <div class="middle">
        ...
        if ($result) { // query was successful
            ...
            while ($row = $statement->fetch()) {
                echo "<tr><td>". $row["id"]
                    ...
                    . "</td><td>" . $row["company"] 
                    . "</td><td>" . $row["year"]
                    . "</td><td><a href='update.php?id=$row[id]&name=$row[name]
                    &price=$row[price]&company=$row[company]&year=$row[year]'>Update</a>
                        <a href='delete.php?id=$row[id]&name=$row[name]&price=$row[price]
                        &company=$row[company]&year=$row[year]'>Delete</a></td>";
                $numRecords++;
            }
            if ($numRecords == 0) {
