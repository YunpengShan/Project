...
let validation = {
    handleEvent(event) {
        ...
        if (form.cheeseWeight.value > 15 && form.weightUnits.value == "SQUARE" || 
...
let cheese = {
    handleEvent(event) {
        ...
        let units = Units[form.weightUnits.value];
        ...
        let qohText = document.createTextNode(qoh + " ");
        //create text node with cheese weight, and format it to one decimal
        let weightLinkText = document.createTextNode(" " + 
            parseFloat(weight).toFixed(1) + " " + units);
        ...
        div.append(qohText, newCheese.cheeseLink, weightLinkText);
        div.append(myUl);
        form.after(div);
        event.preventDefault();
        }
